...
namespace WindowsFormsApplication1
{
  ...
  return currency;
            ...
            if (mltytrades)
            {
                for (int j = boTradeslist.Count - 1; j >= 0; j--)
                {
                    ...
                    for (var listindex = 0; listindex < valuePair.Count; listindex++)
                    {
                        ...
                        var reclist = CheckMultitradesBack(ctrade,abnTradeslist.Where(x => (x.BOTradeNumber == null)).ToList());
        ...
        private List<CpTrade> CheckMultitradesBack(Ctrade ctrade, List<CpTrade> ABNtrades)
        {
            ...
            if (ctrade != null)
            {
                var possibletrades = ABNtrades.Where(item => (item.BOSymbol == ctrade.symbol_id && item.Price == ctrade.price));
                if (ctrade.qty > 0)
                {
        ...
        private double calculateQtyBack(double? InitialQty, double qty, int i, List<CpTrade> possibletrades, List<long> sequence, int level)
        {
             double nextValue = 0;
            ...
            while ((i < possibletrades.Count) && ((qty) != InitialQty))
            {
                if (Math.Abs(nextValue + qty) <= Math.Abs((double)InitialQty))
                {
                    ...
                    if (qty != InitialQty) qty = calculateQtyBack(InitialQty, qty, i + 1, possibletrades, sequence, level + 1);
                }
                else
                {
                    i++;
        ...
        private IEnumerable<Ctrade> getBOTradesfromList(Dictionary<string, List<Ctrade>> boTradeslist, List<long> listToRemove)
        {
             ...
             //   bool positiveqtyflag = !(trade.Qty < 0);
              ...
              //      if ((boTrades[i].symbol_id == symbol && boTrades[i].price == price) && (boTrades[i].qty > 0 && positiveqtyflag && (Math.Abs((double)boTrades[i].qty) < qtyflag))) possibletrades.Add(boTrades[i]);
                var possibletrades = boTrades.Where(item => (item.symbol_id == symbol && item.price == price)); 
                    if (trade.Qty > 0)
                    {
                        possibletrades = possibletrades.Where(item => (item.qty > 0 && Math.Abs((double) item.qty)<Math.Abs((double)initialQty)));
                        possibletrades = possibletrades.OrderByDescending(o => o.qty);
                    }
                    else
                    {
      ...
      private double calculateQty(double? InitialQty, double qty, int i, List<Ctrade> possibletrades, List<long> sequence, int level)
        {
        ...
        private static List<CpTrade> CreateIdForAbnTrades(List<CpTrade> trades)
        {
            ...
            {
                if ((cpTrade.Type == "OP")&&(cpTrade.BOTradeNumber ==null))
                {
                    var t = 1;
                }
                if (cpTrade.BOTradeNumber == null)
                {
                  ...
                  var key = "";
                    ...
                    {
                        Map symbolvalue;
                        if (ABNMap.TryGetValue(cpTrade.Symbol.Substring(0, cpTrade.Symbol.IndexOf(".")) + cpTrade.Type, out symbolvalue))
                        {
                            var daystring = "";
        ...
        private static Dictionary<string, List<Ctrade>> CreateIdForBoTrades(List<Ctrade> boTradeslist)
        {
            ...
            */
            ...
            foreach (Ctrade botrade in boTradeslist)
            {
                ...
                if (botrade.symbol_id == "I.LIFFE.Z2015.C99_75")
                {
                    var t = 1;
                }
                if (vd == defaltvd)
                    ...
                    {
                        if (botrade.symbol_id.IndexOf("E2") > -1)
